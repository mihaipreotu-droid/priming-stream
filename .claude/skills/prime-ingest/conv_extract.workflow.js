...
function workerPrompt(conv) {
  return `Record-extraction worker for ONE conversation. You read it whole and write ONE results file.\n\n` +
    ...
    `Build the full list of genuinely notable records in ONE pass of judgement: atomic findings (facts, citations, decisions, local claims) AND the cross-cutting structures. Proportionate to substance — not exhaustive. Each record: summary (per contract), chunk_id (which chunk it anchors to), anchor_start, anchor_end (char offsets into that chunk's BODY, 0 ≤ start ≤ end ≤ body length; locate them per the contract's "Gate 3 — Anchor verification" — snippet + ONE batched find() call per conversation; fallback per Gate 3, don't stall).\n\n` +
    ...
    `## Step 3.9 — Post-draft gates\n` +
    `Run the contract's "Post-draft gates" (Gate 1 compression count, Gate 2 working-session residue, Gate 3 anchor verification) over the FULL draft list BEFORE writing the results file. Draft → gate → emit, as separate passes. Gate 1's word count comes from the SAME batched script as the anchor check — script count > 20 is a hard fail, no tolerance band.\n\n` +
